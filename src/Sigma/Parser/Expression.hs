...
import Sigma.Runtime (returnSlot, runBody)
import Sigma.Types
import Text.Parsec

numOp :: (Double -> Double -> Double) -> Value -> Value -> Value
numOp op (VInt a) (VInt b) =
  let res = op (fromIntegral a) (fromIntegral b)
...
showValue (VMatrix vs) = intercalate "\n" (map showRow vs)
  where
    showRow row = "[" ++ intercalate ", " (map showValue row) ++ "]"
...
factor =
  ( do
    ...
    <|> ( do
            tok <- tokenPrim show update_pos matrizBuiltin
            _ <- lpToken
            rows <- expr
            _ <- commaToken
            cols <- expr
...
functionCall :: SigmaParser Value
functionCall = do
  (name, params, body) <-
    try
      ( do
...
callFunction calleeEnv body = do
  saved <- readIORef returnSlot
  writeIORef returnSlot Nothing
  _ <- runBody calleeEnv body
  result <- readIORef returnSlot
  writeIORef returnSlot saved
  return result
