...
mkTok tc = tokenPrim show update_pos get_tok
  where
...
idToken = tokenPrim show update_pos get_tok
  where
    ...
    get_tok _ = Nothing
intLitToken = tokenPrim show update_pos get_tok
  where
    get_tok tok@(Token _ (IntLit _)) = Just tok
    get_tok _ = Nothing
floatLitToken = tokenPrim show update_pos get_tok
  where
    get_tok tok@(Token _ (FloatLit _)) = Just tok
    get_tok _ = Nothing

boolLitToken :: ParsecT [Token] Env IO Token
boolLitToken = tokenPrim show update_pos get_tok
  where
    get_tok tok@(Token _ (BoolLit _)) = Just tok
    get_tok _ = Nothing

castTypeToken = tokenPrim show update_pos get_tok
  where
...
returnTypeToken = tokenPrim show update_pos get_tok
  where
    get_tok tok@(Token _ TInt) = Just tok
    get_tok tok@(Token _ TFloat) = Just tok
    get_tok tok@(Token _ TBool) = Just tok
...
relopToken = tokenPrim show update_pos get_tok
  where
